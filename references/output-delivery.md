...
**核心原则：压缩包内的目录结构原样保留，用户只确认根目录放在知识库的哪里。**

**Step 1：解压并展示文件清单**

1. 解压到临时目录（`/tmp/tpr-upload-{timestamp}/`）
2. 递归列出所有文件（保留相对路径和目录结构）
3. 向用户展示完整清单：
   ```
   📦 项目资料.zip（共 5 个文件）
...
根据上下文（当前项目、阶段、文件内容）给出 2-3 个建议：

```
压缩包内的目录结构会原样保留。
请确认「项目资料」这个文件夹放在知识库的哪里：

A. TPR/TPR-20260513-001/references/项目资料/  ← 当前项目下
B. TPR/项目资料/                              ← TPR 根目录下
C. 自定义路径：______
...
上传后的实际目录结构：
```
TPR/TPR-20260513-001/references/项目资料/
├── 需求/
│   ├── PRD.md
│   └── 用户调研.md
├── 数据/
...
1. 按确认的根目录，保留压缩包内原始目录结构逐个上传
2. 上传完成后展示实际云端目录和文件清单：
   ```
   ✅ 上传完成（5/5）
   
   知识库路径：个人知识库 / TPR / TPR-20260513-001 / references / 项目资料/
   
   📁 需求/
   ├─ PRD.md         ✅ (fileId: 30001)
   └─ 用户调研.md    ✅ (fileId: 30002)
   📁 数据/
   ├─ 市场数据.csv    ✅ (fileId: 30003)
   └─ 用户画像.xlsx  ✅ (fileId: 30004)
   📁 设计/
   └─ UI规范.pdf     ✅ (fileId: 30005)
   ```
3. 如果有上传失败的文件，单独标注并给出原因
4. 清理临时目录
