...
| **GitHub 本地镜像** | `~/evan-skill-github/tpr-framework/` | 用于向 GitHub 推送的独立 Git 仓库 |
| **RT 管理档案** | `~/evan-skill-github/tpr-framework/RT/RT-001/` | 存放设计文档与发行包 |
| **发行包存储** | `~/evan-skill-github/tpr-framework/RT/RT-001/releases/` | 所有 tarball 版本的物理归档 |

## 3. 同步规则
- **修改代码** → 在 `skills/tpr-framework/` 中进行。
- **打包发布** → 压缩后存入 `RT/RT-001/releases/`。
- **推送 GitHub** → 将最新代码同步到 `~/evan-skill-github/tpr-framework/` 后执行 `git push`。
